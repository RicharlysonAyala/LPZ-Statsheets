...
export default function SubstitutionModal({ role, outPlayer, activeSet, onClose }: Props) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-[#020817]/75 p-4 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="hud-panel-hero tech-frame modal-in w-full max-w-md rounded-[22px] p-6">
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            <h2 id="sub-title" className="font-tech text-xl font-extrabold tracking-wide text-ink text-glow">
              SUBSTITUIÇÃO
            </h2>
            <p className="mt-0.5 text-xs text-muted">
              Troque o jogador sem editar os sets manualmente.
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="btn-press flex h-11 w-11 items-center justify-center rounded-xl border border-danger/30 bg-danger/10 text-danger hover:bg-danger/20"
            aria-label="Fechar"
          >
            <X size={18} />
          </button>
          ...
          {allSets.map((n) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => toggleSet(n)}
                className={`btn-press min-h-11 rounded-xl border px-2 py-2 text-left text-[11px] font-bold ${
                  on
        ...
        <div className="mb-5 flex gap-2">
          <button
            ...
            className="btn-press min-h-11 flex-1 rounded-xl border border-white/10 bg-white/[0.04] py-1.5 text-[11px] font-semibold text-ink hover:border-primary/30 hover:bg-white/[0.08]"
          >
            SÓ SET ATUAL
          </button>
          <button
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={onClose}
            className="btn-press min-h-11 flex-1 rounded-xl border border-danger/40 py-2 text-sm font-bold text-danger hover:bg-danger/10"
          >
            CANCELAR
          </button>
          <button
            type="button"
            onClick={handleApply}
            className="btn-press min-h-11 flex-1 rounded-xl bg-gradient-to-r from-primary to-cyan py-2 text-sm font-bold text-[#03121f] hover:shadow-[0_0_20px_-4px_rgba(56,189,248,0.8)]"
          >
            APLICAR SUB
          </button>
        </div>
